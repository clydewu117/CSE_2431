...
struct memoryMetaData{
  /* this is used in a linked list to go to the next frame */
  struct memoryMetaData * next;

  /* frameNumber is set in setup and is fixed */
  int frameNumber;

  /* this boolean value can be used for the second chance or replacement algorithm */
  bool replacement;

  /* tv is used for traditional least recently used. If using built-in functions with this, compile with -std=gnu99 */
  struct timeval tv;

  /* count is used for least frequently used or most frequently used */
  int count;
};
...
struct processControlBlock{
  ...
  /* pageTable is an array. The index is the page number and what is stored within is the associated frame number. */
  short pageTable[PAGE_TABLE_SIZE];
  bool pageTableValid[PAGE_TABLE_SIZE];

  /* myFrames is a linked list containing the frames owned by this process */
...
short lruPageReplacement(struct processControlBlock * process){
  ...
  struct memoryMetaData *lruFrame = temp;
  while (temp) {
    if (timercmp(&temp->tv, &lruFrame->tv, <)) {
      lruFrame = temp;
    }
    temp = temp->next;
  }
  return lruFrame->frameNumber;
}
...
struct memoryMetaData *temp = process->myFrames;
    struct memoryMetaData *lfuFrame = temp;
    
    while (temp) {
        if (temp->count < lfuFrame->count || 
           (temp->count == lfuFrame->count && timercmp(&temp->tv,
 &lfuFrame->tv, <))) {
            lfuFrame = temp;
        }
        temp = temp->next;
    }
    return lfuFrame->frameNumber;
}
  ...
  while(!done){
    ...
    for(int i = 0; i < NUM_PROCESSES; i++){
      if(remaining[i] > 0){
        ...
        int runTime = (remaining[i] > quantum) ? quantum : remaining[i];
        printf("\nProcess %d runs %d instructions\n", proc[i].procID, runTime);
        simulateBurst(&proc[i], runTime);
        remaining[i] -= runTime;
      }
    }
  }
